...
public class HandPoker extends Hand implements Comparable {
	...
	public void SetBestHand(ArrayList<HandPoker> PossibleHands) {
		...
		//FIXME: Jackson Set the best possible hands and the best made hand.
		//I'll give you one... BestMadeHand... checkout the linq command
		...
		ArrayList<HandPoker> BestPossibleHands = ((Stream<HandPoker>) PossibleHands.stream()
				.map(x -> x.getHandScorePoker())
				.findAny().orElse(null))
				.collect(Collectors.toCollection(ArrayList::new));
		//the end result of this method was to run these two setters:
		
		this.getGP().SetBestPossibleHands(BestPossibleHands.get(0).getHandID(),BestPossibleHands);		
		this.getGP().SetBestMadeHand(BestMadeHand.getHandID(),BestMadeHand);
	}
